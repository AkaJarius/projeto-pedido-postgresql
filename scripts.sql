...
-- DOMÍNIOS:
/*
Um Domínio no PostgreSQL é um tipo de dado personalizado baseado em um tipo existente, com restrições adicionais que definem regras de validação de valores.

Em outras palavras, é como criar um “tipo de dado próprio” que garante consistência e integridade dos dados sempre que for usado em colunas de tabelas.
Tipo de dados: https://www.postgresql.org/docs/current/datatype-numeric.html#DATATYPE-INT

Principais características:

- Baseado em um tipo existente (VARCHAR, INT, NUMERIC, etc.).
- Pode ter restrições (CHECK, NOT NULL).
- Facilita padronização de regras entre várias tabelas.
*/
...
-- Exemplo de utilização de domínio numa tabela:
ALTER TABLE bairro ALTER COLUMN nome TYPE nome_medio;

/*
VIEWS EXCLUÍDAS E RESTAURADAS:
...
-- EXERCÍCIO:
/* O objetivo deste exercício é alterar os tipos de dados dos atributos de todas as tabelas, considerando os domínios criados anteriormente. 
Caso julgue necessário, faça a criação de novos domínios. */
